...
execute if score $temp flor.dummy matches 4.. run tag @s[tag=!flor.know_ignition_rod,scores={flor.knowledge=1..}] add flor.know_ignition_rod
...
execute store result storage flor:temp Items[{Slot:12b}].count int 1 run scoreboard players get $temp flor.dummy
