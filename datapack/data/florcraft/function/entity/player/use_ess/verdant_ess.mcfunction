...
execute if data entity @s equipment.offhand.components."minecraft:custom_data".flor_dat{ess:"verdant"} unless data entity @s equipment.offhand.components."minecraft:custom_data".flor_dat{fill:0} store result score $temp.math flor.dummy run data get entity @s equipment.offhand.components."minecraft:custom_data".flor_dat.fill
...
data modify storage flor:copy root set value {flor_dat:{ess:"verdant"},fill:0}
execute store result storage flor:copy root.flor_dat.fill int 1 run scoreboard players get $temp.math flor.dummy
execute if score $temp.math flor.dummy matches 1.. unless data entity @s equipment.offhand.components."minecraft:custom_data".flor_dat{pulsating_pustule:1b} run item modify entity @s weapon.offhand florcraft:set_cmd/verdant
