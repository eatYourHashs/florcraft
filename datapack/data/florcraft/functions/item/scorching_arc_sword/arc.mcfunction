...
scoreboard players operation $in flor.dummy += $temp flor.dummy
tag @e[dx=5,dz=5,dy=1,distance=0.01..10,sort=nearest,tag=!flor.user,type=!#florcraft:nonattackable,type=!#florcraft:fire_immune,nbt={HurtTime:0s}] add flor.arid_dmg
execute as @e[dx=5,dz=5,dy=1,distance=0.01..10,sort=nearest,tag=!flor.user,type=!#florcraft:nonattackable,type=!#florcraft:fire_immune,nbt={HurtTime:0s}] run function florcraft:utils/damage/damage
