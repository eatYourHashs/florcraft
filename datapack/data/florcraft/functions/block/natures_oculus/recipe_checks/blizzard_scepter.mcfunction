...
execute if score $temp flor.dummy matches 4.. run tag @p[tag=flor.inside_natures_oculus_gui,tag=!flor.know_blizzard_scepter,distance=..12,scores={flor.knowledge=1..}] add flor.know_blizzard_scepter
execute if score $temp flor.dummy matches 4.. run playsound minecraft:block.enchantment_table.use block @a ~ ~ ~ 1 1.5
execute if score $temp flor.dummy matches 4.. run scoreboard players remove @p[tag=flor.inside_natures_oculus_gui,distance=..12,scores={flor.knowledge=1..}] flor.knowledge 1
