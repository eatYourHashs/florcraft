...
execute if score page flor.dummy matches 0 unless data block ~ ~ ~ Items[{Slot:8b}] run item replace block ~ ~ ~ container.8 with structure_block{display:{Name:'{"translate":"block.flor.natures_oculus.standard","color":"white","italic":false}'},BlockEntityTag:{rotation:"NONE",mirror:"NONE",mode:"SAVE"},CustomModelData:423002,flor_dat:{next_button:1b,cleared:1b}}
