...
execute if score $out flor.verdant_ess matches 2.. if entity @s[predicate=florcraft:item/druidic_lauret] run tag @e[sort=nearest,limit=1,tag=flor.snare_scepter] add flor.buffed_magic_15
execute if score $out flor.verdant_ess matches 2.. if entity @s[predicate=florcraft:item/verdant_mastery_circlet] run tag @e[sort=nearest,limit=1,tag=flor.snare_scepter] add flor.buffed_magic_30