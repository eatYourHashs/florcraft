execute store result score $temp flor.dummy run data get storage flor:temp Items[{Slot:23b}].count
...
execute store result storage flor:temp Items[{Slot:23b}].count byte 1 run scoreboard players get $temp flor.dummy
...
data modify entity @e[type=item,tag=flor.new_drop,limit=1] Item set from storage flor:temp Items[{Slot:23b}]
