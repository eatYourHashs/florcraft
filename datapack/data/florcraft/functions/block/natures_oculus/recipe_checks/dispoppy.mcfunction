data modify block -30000000 0 24576 Items prepend from block ~ ~ ~ Items[{Slot:16b}]
execute store result score $temp flor.dummy run data get block -30000000 0 24576 Items[0].Count
execute if score $temp flor.dummy matches 8.. run tag @p[tag=flor.inside_natures_oculus_gui,tag=!flor.know_dispoppy,distance=..12,scores={flor.knowledge=1..}] add flor.know_dispoppy
execute if score $temp flor.dummy matches 8.. run playsound minecraft:block.enchantment_table.use block @a ~ ~ ~ 1 1.5
execute if score $temp flor.dummy matches 8.. run scoreboard players remove @p[tag=flor.inside_natures_oculus_gui,distance=..12,scores={flor.knowledge=1..}] flor.knowledge 1
