...
execute if data entity @s SelectedItem.components."minecraft:custom_data".flor_dat{ess:"verdant"} run scoreboard players operation $temp flor.dummy += @e[type=armor_stand,distance=..0.6,tag=flor.pylon_variant] flor.verdant_ess
execute if data entity @s SelectedItem.components."minecraft:custom_data".flor_dat{ess:"verdant"} run scoreboard players set @e[type=armor_stand,distance=..0.6,tag=flor.pylon_variant] flor.verdant_ess 0
