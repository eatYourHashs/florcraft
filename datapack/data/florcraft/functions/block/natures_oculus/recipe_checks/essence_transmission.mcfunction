...
execute if score $temp flor.dummy matches 6.. run tag @s[tag=!flor.know_essence_transmission,scores={flor.knowledge=1..}] add flor.know_essence_transmission
...
execute store result storage flor:temp Items[{Slot:15b}].count int 1 run scoreboard players get $temp flor.dummy
