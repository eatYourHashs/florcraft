...
execute if entity @e[dx=0,dz=0,dy=0,distance=..5,sort=arbitrary,tag=!smithed.entity,tag=!flor.user,nbt={HurtTime:0s}] run scoreboard players set $cast.temp flor.dummy 9999
...
execute as @e[dx=0,dz=0,dy=0,distance=..5,sort=arbitrary,tag=!smithed.entity,tag=!flor.user,type=!#florcraft:nonattackable,type=!#florcraft:fire_immune,tag=!flor.invoked_arid,nbt={HurtTime:0s}] run function florcraft:item/ignition_rod/damage
