...
execute if data entity @s equipment.offhand.components."minecraft:custom_data".flor_dat{essence_item:2b} store result score $temp.math flor.dummy run data get entity @s equipment.offhand.count

#subtract the input amount of essence from score
scoreboard players operation $temp.math flor.dummy -= $in flor.aqueous_ess

#if raw ess, replace and stop
execute if data entity @s equipment.offhand.components."minecraft:custom_data".flor_dat{essence_item:2b} run return run loot replace entity @s weapon.offhand loot florcraft:item/with/aqueous_essence_from_score

#if pulsating pustule and xp needs to be used, do shit for that
execute if data entity @s equipment.offhand.components."minecraft:custom_data".flor_dat{pulsating_pustule:1b} if score $temp.math flor.dummy matches ..-1 run function florcraft:item/pulsating_pustule/use_xp_aqueous
