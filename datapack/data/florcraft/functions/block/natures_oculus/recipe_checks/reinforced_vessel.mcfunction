...
execute if score $temp flor.dummy matches 6.. run tag @s[tag=!flor.know_reinforced_vessel,scores={flor.knowledge=1..}] add flor.know_reinforced_vessel
...
data modify entity @e[type=item,tag=flor.new_drop] Item set from storage flor:temp Items[{Slot:10b}]
