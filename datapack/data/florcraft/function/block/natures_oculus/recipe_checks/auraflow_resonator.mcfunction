...
execute if score $temp flor.dummy matches 6.. run tag @s[tag=!flor.know_auraflow_resonator,scores={flor.knowledge=1..}] add flor.know_auraflow_resonator
...
execute store result storage flor:temp Items[{Slot:20b}].count int 1 run scoreboard players get $temp flor.dummy
