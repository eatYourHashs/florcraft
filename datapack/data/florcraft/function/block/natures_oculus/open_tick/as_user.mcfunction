...
execute if score page flor.dummy < @s flor.pages unless data block ~ ~ ~ Items[{Slot:8b}] run item replace block ~ ~ ~ container.8 with structure_block[minecraft:item_name={"translate":"block.flor.natures_oculus.next","color":"white","italic":false},minecraft:item_model="florcraft:right_arrow",minecraft:custom_data={flor_dat:{next_button:1,cleared:1b}}]
