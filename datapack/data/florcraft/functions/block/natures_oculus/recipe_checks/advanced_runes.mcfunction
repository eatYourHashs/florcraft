data modify block -30000000 0 24576 Items prepend from block ~ ~ ~ Items[{Slot:10b}]
execute store result score $temp flor.dummy run data get block -30000000 0 24576 Items[0].Count
execute if score $temp flor.dummy matches 8.. run tag @p[tag=flor.inside_natures_oculus_gui,tag=!flor.know_advanced_runes,distance=..12,scores={flor.knowledge=2..}] add flor.know_advanced_runes
execute if score $temp flor.dummy matches 8.. run playsound minecraft:block.enchantment_table.use block @a ~ ~ ~ 1 1.5
execute if score $temp flor.dummy matches 8.. run scoreboard players set @p[tag=flor.inside_natures_oculus_gui,distance=..12,scores={flor.knowledge=2..}] flor.pages 4
execute if score $temp flor.dummy matches 8.. run scoreboard players remove @p[tag=flor.inside_natures_oculus_gui,distance=..12,scores={flor.knowledge=2..}] flor.knowledge 2
