...
execute if score $temp flor.dummy matches 10.. run tag @s[tag=!flor.know_essence_flask,scores={flor.knowledge=1..}] add flor.know_essence_flask
...
execute store result storage flor:temp Items[{Slot:9b}].count int 1 run scoreboard players get $temp flor.dummy
...
data modify entity @e[type=item,tag=flor.new_drop,limit=1] Item set from storage flor:temp Items[{Slot:9b}]
