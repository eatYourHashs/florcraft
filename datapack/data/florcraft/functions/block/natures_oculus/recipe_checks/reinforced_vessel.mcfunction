...
execute if score $temp flor.dummy matches 6.. run tag @p[tag=flor.inside_natures_oculus_gui,tag=!flor.know_reinforced_vessel,distance=..12,scores={flor.knowledge=1..}] add flor.know_reinforced_vessel
execute if score $temp flor.dummy matches 6.. run playsound minecraft:block.enchantment_table.use block @a ~ ~ ~ 1 1.5
execute if score $temp flor.dummy matches 6.. run scoreboard players remove @p[tag=flor.inside_natures_oculus_gui,distance=..12,scores={flor.knowledge=1..}] flor.knowledge 1
