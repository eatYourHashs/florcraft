...
execute if score $temp flor.dummy matches 6.. run tag @s[tag=!flor.know_reinforced_pylons,scores={flor.knowledge=1..}] add flor.know_reinforced_pylons
...
execute store result storage flor:temp Items[{Slot:11b}].count int 1 run scoreboard players get $temp flor.dummy
