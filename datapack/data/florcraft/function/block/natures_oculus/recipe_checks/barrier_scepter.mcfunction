...
execute if score $temp flor.dummy matches 2.. run tag @s[tag=!flor.know_barrier_scepter,scores={flor.knowledge=1..}] add flor.know_barrier_scepter
...
execute store result storage flor:temp Items[{Slot:14b}].count int 1 run scoreboard players get $temp flor.dummy
