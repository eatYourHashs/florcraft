...
execute if score $temp flor.dummy matches 6.. run tag @s[tag=!flor.know_upgrade_seals,scores={flor.knowledge=1..}] add flor.know_upgrade_seals
...
execute store result storage flor:temp Items[{Slot:17b}].count int 1 run scoreboard players get $temp flor.dummy
