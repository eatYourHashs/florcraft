execute store result score $temp flor.dummy run data get storage flor:temp Items[{Slot:16b}].Count
execute if score $temp flor.dummy matches 3.. run tag @s[tag=!flor.know_exobiotic_armor,scores={flor.knowledge=1..}] add flor.know_exobiotic_armor
...
execute store result storage flor:temp Items[{Slot:16b}].Count byte 1 run scoreboard players get $temp flor.dummy
...
data modify entity @e[type=item,tag=flor.new_drop,limit=1] Item set from storage flor:temp Items[{Slot:16b}]
