...
execute store result score $temp.ess flor.dummy run data get storage flor:copy root.flor_dat.exobiotic_leggings
...
execute if entity @s[tag=flor.item_changed] store result storage flor:copy root.flor_dat.dmg int 1 run scoreboard players get $temp.dmg flor.dummy
execute if entity @s[tag=flor.item_changed] run item modify entity @s armor.head florcraft:copy_custom_data
execute if entity @s[tag=flor.item_changed] if score $temp.fill flor.dummy matches ..0 run function florcraft:item/exobiotic/helmet/deactivate
