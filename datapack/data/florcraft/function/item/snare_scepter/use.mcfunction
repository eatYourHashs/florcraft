...
execute if score $out flor.verdant_ess matches 2.. run summon armor_stand ~ ~ ~ {NoGravity:1b,Invulnerable:1b,ShowArms:0b,Small:0b,Marker:1b,Invisible:1b,NoBasePlate:1b,PersistenceRequired:1b,Tags:["smithed.entity","flor.ticking","flor.snare_scepter"]}
...
execute if score $out flor.verdant_ess matches 2.. if entity @s[predicate=florcraft:item/verdant_exobiotic_helmet] run tag @e[sort=nearest,limit=1,tag=flor.snare_scepter] add flor.buffed_magic_40
